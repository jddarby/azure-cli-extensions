...
class OnboardingBaseInputConfig(ABC):
    ...
    publisher_name: str = field(
        metadata={
            "comment:": "Name of the Publisher resource you want your definition published to. Will be created if it does not exist."
        }
    )
    publisher_resource_group_name: str = field(
        metadata={
            "comment": "Optional. Resource group for the Publisher resource. Will be created if it does not exist (with a default name if none is supplied)."
        }
    )
